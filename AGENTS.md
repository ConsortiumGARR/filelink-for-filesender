...
- All paths inside the extension (manifest, `windows.create`, `service_icon`) are
  ...
  by the manifest, the background scripts and the pages exists.

## Formatting

- Python (`test/`, `tools/`): black (88 columns) for formatting, ruff for lint only (`E
  F W I B UP`, E501 ignored because of embedded JS strings). Settings in
  `pyproject.toml`; `reference/` is excluded. Tool versions are pinned in
  `pyproject.toml`'s `[dependency-groups] dev` and `uv.lock`, not in CI YAML: `uv sync
  --group dev` (used by `.github/workflows/test.yml`) installs exactly those into
  `.venv/`. `tools/format.sh` still just runs bare `black`/`ruff`, wherever they are on
  PATH; it does not require `uv` locally.
- JS/HTML/JSON/CSS of the extension: Prettier 3 (`.prettierrc.json`: single quotes, 100
  columns, trailing commas). Markdown is not formatted by Prettier: docs are wrapped by
  hand at 88 columns (tables excepted).
...
- `sh tools/format.sh` formats everything, `--check` only verifies (also run by `sh
  ...
  `node:22-alpine` container, since Node is not installed here.

## Testing (no Node in this environment)

- Everything at once: `sh test/check.sh` (includes the formatting check).
- Static checks: `python3 test/static_check.py` (JS syntax, locale parity, placeholders,
  i18n keys used/unused, referenced files exist in src/).
- Signing: `python3 test/jsc_run.py` after any change to `src/lib/filesender.js`. Needs
  python3-gi (JavaScriptCore 4.1). It shims btoa/atob/TextEncoder, runs known-answer
  tests, then checks the library against `test/vectors.json` (7/7 KATs, 6/6
  differential).
- Ground-truth vectors live in `test/reference.py` (a Python mirror of `filesender.py`),
  generated by `test/gen.py` into `test/vectors.json`. If you must change the signing,
  keep the reference and the vectors in sync with filesender.py.
- Requests: `python3 test/request_sim.py` (fake fetch/timers/AbortSignal: Retry-After,
  retry statuses, no retry on 500, timeouts, abort during backoff).
- Background logic: `python3 test/background_sim.py` after any change to
  `src/background/`. It loads the background scripts in manifest order in JSC with fake
  `browser.*` APIs and a fake `globalThis.filesender`, then drives scenarios (direct
  upload, terms, empty, too large, bad name, reuse long/short/invalid/changed/encrypted,
  options batch, cancel, window closed, abort while pending, bad password, ignored
  options, credential check, cleanup of unsent mails, redaction, window centering). Add
  a scenario when you add a branch.
- Real WebCrypto: `python3 test/gen.py && sh test/browser_run.sh` after changes to
  `src/lib/filesender.js` or `src/lib/fscrypto.js`. Runs `test/sign-test.html` and
  `test/crypto-test.html` in headless Chrome. Crypto vectors come from
  `test/crypto_reference.py`, a copy of the filesender.py encryption (needs python
  `cryptography`).
