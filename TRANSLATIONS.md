...
3. Run `python3 test/static_check.py` (or `sh test/check.sh`). It fails if a locale is
   missing a key the other has, or uses a `$NAME$` placeholder that is not declared.
